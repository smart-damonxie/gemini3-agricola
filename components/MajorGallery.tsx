...
const MajorGallery: React.FC<Props> = ({ availableMajors, onClose, onSelect }) => {
    return (
            ...
            <div className="bg-stone-900 border-4 border-yellow-700 rounded-xl p-6 max-w-6xl w-full h-[90vh] flex flex-col shadow-2xl relative" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6 pb-2 border-b border-yellow-700/50">
                    <h2 className="text-3xl font-bold text-yellow-500 tracking-wider">
                        {onSelect ? "Select a Major Improvement" : "Major Improvements Gallery"}
                    </h2>
                ...
                <div className="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 p-4 scrollbar-thin scrollbar-thumb-stone-600">
                    {DB_MAJORS.map(major => {
                        ...
                        const isSelectable = isAvailable && !!onSelect;
                        
                        return (
                            <div 
                                key={major.id} 
                                onClick={() => { if (isSelectable) onSelect(major.id); }}
                                className={`
                                    relative group rounded-lg overflow-hidden border-2 transition-all duration-300 
                                    ${isSelectable ? 'cursor-pointer hover:border-green-500 hover:scale-105 hover:shadow-2xl hover:ring-2 hover:ring-green-400' : ''}
                                    ${!isSelectable && isAvailable ? 'border-yellow-600/50' : ''}
                                    ${!isAvailable ? 'border-stone-800 bg-stone-950 opacity-60' : ''}
                                `}
                            >
                                <div className="aspect-[2/3] bg-stone-800 relative flex items-center justify-center">
                                    ...
                                    {/* Select Overlay */}
                                    {isSelectable && (
                                        <div className="absolute inset-0 bg-green-900/0 group-hover:bg-green-900/20 transition-colors flex items-center justify-center">
                                            <span className="opacity-0 group-hover:opacity-100 bg-green-600 text-white font-bold px-3 py-1 rounded shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all">
                                                Click to Select
                                            </span>
                                        </div>
                                    )}
